...
function matchScoreTotal(result){
  ...
  prompt(`The score is ${userScore} to ${computerScore}.`);
}

function matchOver(userScoreTotal, computerScoreTotal) {
  if (userScoreTotal === 5) {
    prompt(`\n ${lineDecoration}\n  Match over. You have won ${userScoreTotal} games.\n ${lineDecoration}`)
    userScore = 0;
    computerScore = 0;
  } else if (computerScoreTotal === 5) {
    prompt(`\n ${lineDecoration}\n  Match over. The computer has won ${computerScoreTotal} games. \n ${lineDecoration}`)
    userScore = 0;
    computerScore = 0;
  }
}
...
while (true) {
  ...
  displayGameWinner(userChoiceFunction(), computerChoiceFunction());
